...
с главной страницы, копируем ее свойства в новую кнопку и изменяе согласно макету(изменился цвет текста и рамки), размер кнопки увеличился на 10пкс. Из-за высоты поменяется
...
При добавлении ссылки появились подчеркивания на под текстом, нужно это убрать путес стилизации. Для этого присваиваем ссылке класс class="card-link"  и стилизуем ее в ксс
...
Стилизуем ссылку при наведении, прописываем селектор .card-link:hover {    color: #012210; - добавляем цвет текста при наведении но в верстке на сайте это не рсабатывает.
...
включает в себя контент, но цвет текста по прежнему не меняется. Теперь протестируем ховер, в панали разработчика(инспекторе) в вкладке styles напротив filter есть
...
цвет в них, и они идут в ксс файле после ховера и считаются главнее. Убираем єти цвета в .card-title { и .card p { так как главный цвет по умолчанию у нас прописан в боди.
...
Реализуем  ссылку на второй странице. Начнем с баттн-бэк пропишем для нее свойства .project-btn-back:hover - при наведении.  background-color: white; - заливка кнопки
...
Для мобильной адаптивности будем использовать медиа запросы. Пока увеличи ширину до 1200 пкс и будем наблюдать как она будет менятся.
...
В середине медиазапроса указываем блок в котором мы хотим поменять свойства, а в этом блоке пишет или новые свойства которые хотим добавить или те из старых свойств которые
...
56 пск. Также изменим подзаголовок на 14пкс вместо 16.
...
не переносятся на следующую строку, но картинка вылазит за пределы карточки. У меня не получилось что бы между карточками был зазор при уменьшении размера, они слипались,
поэтому я уменьшил паддинги на контейнере до 10ти а для карточек сделал марджены по 5пкс.


Сделаем ограничение картинка рамками карточки, что бы она изменяла размер вместе с карточкой. Пропишем в мейн ксс селектор для всех картинок img { max-width: 100%;
...
Также при этом размере и меньше мы уже хотив видеть карточки по одной в ряд, для этого пропишем .card { width: 100%;. Теперь карточка одна но она не растет больше макс ширины
540пкс и поэтому находится не по центру. Так как карточку находятся в .portfolio-card-wrapper то для него в медиа 760 пропишем justify-content: center; теперь карточки
...
Мы отменяли отступы вниз у последних двух карточек, но теперь когда они по одной в ряду, предпоследняя слипается с полседней, тоесть нужно отпенить отступ только у последней.
...
.footer-column:last-child{margin-bottom: 0; - но у нас последний элемента теперь первым стоит и нужно выбрать ферст чайлд  .footer-column:first-child{margin-bottom: 0;
...
В контактах тоже меняе шрифт и отступ вверх обнуляя другие отступы .footer-contacts p {font-size: 14px; margin: 0; margin-top: 15px;

Уменьшаю отступы в середине кнопки потому что текст вылазит за ее пределы для разрешения .footer-button { padding: 0 10px ; width: 100%; max-width: 320px;


Удаляем все классы ноне и перейдем к редактированию второй страницы(внутренней).
...
.project-btn-back { padding: 0 10px ;  также зададим ширину 100% а макс ширину в 320пкс это мы задаем для того что бы на разрешении от 760 до 320 пкс кнопка не прилягала
в плотную к тексту в середине и лучше смотрелась. width: 100%; max-width: 320px;
